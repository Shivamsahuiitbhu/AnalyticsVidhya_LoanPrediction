...
cv = cross_validation.KFold(len(df), n_folds=5,random_state=1)
error = []
roc = []
...
for traincv, testcv in cv:
    ...
    train_predictors = (df.iloc[traincv,:])
    
    # The target we're using to train the algorithm.
    train_target = target_variable.iloc[traincv]
    
    # Training the algorithm using the predictors and target.
    model.fit(train_predictors, train_target.values.ravel())
    
    pred = model.predict(df.iloc[testcv,:])
    
    #Record error from each cross-validation run
    error.append(metrics.accuracy_score(pred, target_variable.iloc[testcv]))
    roc.append(metrics.roc_auc_score(target_variable.iloc[testcv],pred))


pred_train = model.predict(train)
accuracy  = metrics.accuracy_score(pred_train,target)

print("model performance")
print("Accuracy : %s" % "{0:.4%}".format(accuracy))
print("Cross-Validation Score : %s" % "{0:.4%}".format(np.mean(error)))
print("ROC_Score :%s" % "{0:.4%}".format(np.mean(roc)))
